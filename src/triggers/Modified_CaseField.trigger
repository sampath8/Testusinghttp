trigger Modified_CaseField on Contact (after update) {
    ...
    if ( flag ){
        List<Case> caselst=[Select Status,Agent_Assigned__c,SuppliedPhone,Notes__c,Reason_for_Moving__c,Move_in_Date__c,SuppliedEmail,
                            ...
                            PC_Integration_Fail_Success_Message__c,ResMan_Success_Error_Message__c,ResMan_Integration_Failed__c,ResMan_GC_Entered__c from Case where ContactId=:conNew.Id order by CreatedDate desc limit 1];
